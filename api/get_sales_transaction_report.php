...
try {
    ...
    if (empty($from) && empty($to)) {
        ...
        $sql = "SELECT * FROM sales_tbl WHERE status = 'Completed' ORDER BY sales_transaction_id DESC";
        $stmt = $db->prepare($sql);
        $stmt->execute();
        ...
        $sql = "SELECT * FROM sales_tbl WHERE status = 'Completed' AND transaction_date BETWEEN :dateFrom AND :dateTo ORDER BY sales_transaction_id DESC";
        $stmt = $db->prepare($sql);
